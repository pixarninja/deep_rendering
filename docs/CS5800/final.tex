...
higher precision models can be applied to shrink frameblock resolution further.
...
\draw (-1,0) -- (3,0); % ZQ
\draw (-1,0) -- (2,-1); % ZR
\draw[dashed] (1.5,1) -- (2,-1); % PR
\draw[dashed] (1.5,1) -- (3,0); % PQ
\draw[dashed] (3,0) -- (2,-1); % QR
%%Labels for the vertices are typeset.
\node[below left = -2.45mm of {(1.5,1)}] {\textbullet}; % P
\node[below left = -2.35mm of {(3,0)}] {\textbullet}; % Q
\node[below left = -2.5mm of {(2,-1)}] {\textbullet}; % R
\node[right = 1mm of {(1.5,1.15)}] {$P$};
\node[right = 1mm of {(3,0)}] {$Q$};
\node[right = 1mm of {(2,-1)}] {$R$};
